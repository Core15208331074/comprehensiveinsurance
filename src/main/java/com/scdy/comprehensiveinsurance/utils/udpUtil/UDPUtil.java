package com.scdy.comprehensiveinsurance.utils.udpUtil;

import com.alibaba.fastjson.JSONObject;
import com.scdy.comprehensiveinsurance.utils.commonUtil.CommonUtil;
import lombok.Data;
...
//    }
    ...
    public static void sendData(byte[] bytes) {
        ...
        DatagramPacket packet = new DatagramPacket(bytes, bytes.length, net, 9999);
        ...
        if (CommonUtil.isNotNull(dataArray) && dataArray.length > 0) {
            ...
            for (int i = 0; i < dataArray.length; i++) {
                if (i <= 5) {//帧头
                    ...
                    channelJudge.append(dataArray[i]);
                    channelJudge.append(" ");
